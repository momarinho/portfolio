    ...
    return (
        <section id="skills" className="py-12 md:py-20 px-4 max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-center items-start gap-8 md:gap-16 relative">
                ...
                {/* Core Magic */}
                <SkillNode title={t.skills.core} color="text-[#b026ff]" skills={skillsList.core} icon="🔮" delay={0} />

                {/* Support */}
                <SkillNode title={t.skills.support} color="text-[#00f3ff]" skills={skillsList.support} icon="🛡️" delay={0.2} />

                {/* Frontline */}
                <SkillNode title={t.skills.frontline} color="text-[#ffd700]" skills={skillsList.frontline} icon="⚔️" delay={0.4} />
            </div>
        </section>
    );
}

function SkillNode({ title, color, skills, icon, delay }: { title: string, color: string, skills: string[], icon: string, delay: number }) {
    return (
        <motion.div
        ...
        >
            ...
            <h3 className={`font-press-start text-center mt-6 mb-6 ${color} text-lg`}>{title}</h3>

            <ul className="space-y-3 font-vt323 text-xl">
                {skills.map(skill => (
                    <li key={skill} className="flex items-center gap-2 group">
                        <span className="opacity-50 group-hover:opacity-100 group-hover:text-[#39ff14] transition-all">►</span>
